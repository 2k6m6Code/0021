...
my $QB_FILE_XMLCONF="/usr/local/apache/qbconf/file.xml";
my $QB_SQUID_BLKFILE=$QB_SQUID."blockfile.txt";

#------------------------------------------------------------------
# read the option from the file  /usr/local/apache/qbconf/squidgen.xml
#------------------------------------------------------------------
my $fileref=XMLread($QB_FILE_XMLCONF);

if ( !$fileref ) #if the string is NULL
{
...
$filelist = $fileref->{file};
my $blockfile = '';
foreach my $item ( @$filelist ) 
{
    if ( $item->{block} eq '1' )
    {
        $blockfile .= "\\\.$item->{filename}\$\n";
    }
}
print BLKFILE qq "$blockfile";
close(BLKFILE);
=cut
