...
{    
	...
	\$("#query").attr('disabled', true);
     var url = "search_data_quota_total.pl";
...
function search_quota_detail(start,end,id)
{
	\$("#load_gif").css('display','block');
	var start = start;
	var end = end;
	var id = id;
	var url = "search_data_quota.pl";
	\$.get(url,{start:start,end:end,id:id},function fno(data){
		\$("#table").html(data);
	     	var oTable =  \$('#tables').dataTable({
			"timeout": 5000,
			"bPaginate": false,
