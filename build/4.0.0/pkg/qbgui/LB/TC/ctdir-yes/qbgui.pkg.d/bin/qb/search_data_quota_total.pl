...
t1:foreach my $data (@file)
{
	...
	print qq ($start,$end,"$data_splic[0] $data_splic[1] $data_splic[2] $my_year[3]"<br>);
    ...
    #{   
		...
		#	if (time_2($start,$end,"$data_splic[0] $data_splic[1] $data_splic[2] $my_year[3]"))
		#	{
				if ($id eq $data_splic[2] ){$count++;}
		#	}
		}
		#print qq (<td></td>);
		print qq(<td style="width: 200px;" onclick="javascript:search_quota_detail('$start','$end','$id')">$count</td>); 
        print qq (</tr>);
		$id='';
        $top++;
    #}
