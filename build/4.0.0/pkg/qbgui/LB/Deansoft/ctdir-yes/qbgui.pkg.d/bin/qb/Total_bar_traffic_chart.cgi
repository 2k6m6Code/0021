...
//alert('Index: ');
  \$("#iptext").val(\$("#sidd").val());
			...
			var fkd = (10 - data.length)/2;
			var sdh;
				for( sdh = 0;sdh<fkd;sdh++){
				newdljson.push({data:0,label:"-Download"});
				}
				newdljson = newdljson.concat(data);
			...
			\$("#dtt").text("TOP "+data.length+" Download");
				for(var dd = 0;dd<data.length;dd++){
				data[dd].data =[[data[dd].data,data.length-dd ]];
				tick_labels.push([data.length-dd,data[dd].label.replace("-Download","")]);
				}
		...
		if(data.length>10){
			...
			var sdh;
				for( sdh = 0;sdh<fkd;sdh++){
				newdljson.push({data:0,label:"-Upload"});
				}
				newdljson = newdljson.concat(data);
				for( sdh = newdljson.length;sdh<10;sdh++){
				newdljson.push({data:0,label:"-Upload"});
		...
		\$("#utt").text("TOP "+data.length+" Upload");
		for(var dd = 0;dd<data.length;dd++){
				data[dd].data =[[data[dd].data,data.length-dd ]];
				tick_labels.push([data.length-dd,data[dd].label.replace("-Upload","")]);
				}
...
function eventshow(event, pos, item) {
    if (item) {
        if (previousPoint != item.datapoint) {
            ...
            showTooltip(item.pageX+5, item.pageY+5, x+" (Kb)",item.series.color);
