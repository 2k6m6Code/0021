...
modifyfile($QB_3PROXY_CONF, "^internal.*", "internal $local_interface"); 
modifyfile($QB_3PROXY_CONF, "^external.*", "external 172.31.3.1"); 
modifyfile($QB_3PROXY_CONF, "^tcppm.*", "tcppm $port $target_ip $port"); 
...
#print PROXY qq "/usr/local/sbin/iptables -t nat -A PREROUTING -p tcp --dport $port -j DNAT --to-dest 172.31.3.1:$port\n";
#-------------------------------------------------
# remark 
#-------------------------------------------------
my $num=0;
